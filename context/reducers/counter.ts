...
type CounterPayloadMap = {
    ...
    [CounterActionTypes.IncrementAsyncByAmount]: { },
    [CounterActionTypes.IncrementIfOdd]: { amount: number },
    [CounterActionTypes.IncrementAsyncByAmountSuccess]: { amount: number },
    [CounterActionTypes.IncrementAsyncByAmountFailure]: { amount: number },
}

export type CounterActions = ActionMap<CounterPayloadMap>[keyof ActionMap<CounterPayloadMap>];

export function counterReducer(state: CounterState = counterInitialState, action: CounterActions): CounterState {
  switch (action.type) {
    ...
    case CounterActionTypes.IncrementIfOdd: 
        return {
            ...state,
            value: state.value % 2 === 1 ? state.value + action.payload.amount : state.value,
        }
    default:
        return state;
  }
